  ...
  quotes: router({
    ...
    sendEmail: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const portalLink = `${input.origin}/public/quote/${quote.publicToken}`;
  ...
  invoices: router({
    ...
    markPaid: publicProcedure
      .input(
        z.object({
          token: z.string(),
          paidAmount: z.number().or(z.string()),
        })
      )
      .mutation(async ({ input }) => {
        const invoice = await db.getInvoiceByPublicToken(input.token);
    ...
    sendEmail: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const pdfBuffer = await pdfDocumentToBuffer(pdfDoc);
        const portalLink = `${input.origin}/public/invoice/${invoice.publicToken}`;

...
export default function JobForm() {
  const route = useRoute("/app/jobs/:action/:id?");
  const [, setLocation] = useLocation();
  const [isLoading, setIsLoading] = useState(false);
  const [photos, setPhotos] = useState<PhotoItem[]>([]);

  const isEditMode = route ? (route[1] as any)?.action === "edit" : false;
  const jobId = route && isEditMode ? parseInt(((route[1] as any)?.id as string) || "0") : undefined;
